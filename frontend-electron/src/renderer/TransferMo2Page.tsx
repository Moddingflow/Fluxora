    ...
    <section className="transfer-drive-section" aria-label="Диски назначения">
      <header>
        <div>
          ...
          <span>
            {selectedDrive
              ? `${selectedDrive.mediaLabel} - доступно ${formatTransferBytes(selectedDrive.availableBytes)}`
              : 'Выберите диск для новой копии сборки'}
          </span>
        </div>
        <button
  ...
  const renderSetup = () => (
    <div className="transfer-page-panel transfer-page-panel--setup">
      ...
      <header className="transfer-page-hero">
        ...
        <div className="transfer-page-status">
          <strong>{busyLabel || (driveState === 'loading' ? 'Обновляем диски' : 'Готово к настройке')}</strong>
      ...
      {!cancellationSupported ? (
        <small className="transfer-cancel-note">
          Отмена будет отправлена в bridge. Если текущая native-сборка не поддерживает отмену,
          Fluxora сохранит перенос заблокированным до безопасного завершения.
        </small>
      ) : null}
    </div>
  );
  ...
  const renderProgress = () => {
    ...
    return (
      <div className="transfer-page-panel transfer-page-panel--progress">
        ...
        <div className="transfer-operation-note">
          <ShieldCheck size={17} aria-hidden="true" />
          <span>Оригинальная папка MO2 не изменяется. Fluxora работает с новой копией в выбранном месте.</span>
        </div>
