...
namespace fluxora
{
    namespace
    {
        ...
        std::vector<ExecutableLaunchMod> activeProfileLaunchModsFromRecords(
            const ProjectExecutableContext& context,
            const std::vector<ProfileOrderItemRecord>& records)
        {
            std::vector<ExecutableLaunchMod> snapshot;
            std::vector<std::filesystem::path> paths;
            snapshot.reserve(records.size());
            paths.reserve(records.size());
            for (const ProfileOrderItemRecord& item : records)
            {
                if (item.kind != L"mod" ||
                    !item.hasMod ||
                    item.mod.state == L"disabled" ||
                ...
                paths.push_back(item.mod.path);
                snapshot.push_back(ExecutableLaunchMod{
                    item.mod.path,
                    item.mod.displayName.empty() ? item.mod.folderName : item.mod.displayName,
                    item.mod.contentFingerprint
                });
            }

            context.activeProfileModPathsCache = std::move(paths);
            return snapshot;
        }

        std::vector<ExecutableLaunchMod> reconcileActiveProfileLaunchMods(
            const ProjectExecutableContext& context)
        {
            if (context.projectDirectory.empty() || context.modsDirectory.empty())
            {
            ...
            const std::wstring profile = context.defaultProfile.empty() ? L"Default" : context.defaultProfile;
            return activeProfileLaunchModsFromRecords(
                context,
                InstanceMetadataStore::listLaunchProfileOrderItems(
                    context.projectDirectory,
                    profile,
                    context.modsDirectory));
        }
        ...
        std::filesystem::path prepareParallaxGenMo2Metadata(
            ...
            const std::filesystem::path& outputMod,
            Logger& logger,
            std::vector<ExecutableLaunchMod>* activeProfileMods = nullptr)
        {
            if (context.gamePath.empty())
            {
            ...
            writeUtf8TextFile(
                ...
                parallaxGenMo2ModList(context, outputMod, activeProfileMods),
        ...
        std::wstring_view additionalArguments) const
    {
        ...
        // The same launch-consistent snapshot drives Root Builder resolution
        // below and the later VFS mount plan. Storage errors deliberately
        // propagate: silently launching with an empty profile would be a
        // correctness failure.
        std::vector<ExecutableLaunchMod> activeProfileMods;
        if (supportsRootBuilder(context) || context.vfsRules.has_value())
        {
            activeProfileMods = reconcileActiveProfileLaunchMods(context);
        }
        ...
        const std::optional<std::filesystem::path> parallaxGenOutputMod =
            prepareParallaxGenOutputForLaunch(
                context,
                *match,
