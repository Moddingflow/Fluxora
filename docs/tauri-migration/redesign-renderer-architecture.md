...
| Application update | `features/update/AppUpdateToolbarButton.tsx`, `features/update/AppUpdateWindow.tsx`, `features/update/app-update-progress.ts`, `installer/components/UpdateStageProgressPanel.tsx` | The main shell exposes availability only; the isolated renderer window owns two-stage download/install presentation, transfer-speed sampling, retry/dismiss UI and typed update-status consumption. Network, filesystem, process, drain and transaction authority remain native. |
