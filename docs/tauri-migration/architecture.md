...
- Product UI copy is renderer-owned and bundled through i18next JSON catalogs in `frontend-tauri/src/localization/locales/{en-US,de-DE,ru-RU}.json`; TypeScript and JSX consume typed keys rather than embedding translations. `en-US` is the fallback/source catalog, every locale must keep exact key and interpolation-placeholder parity, and React/non-React consumers use the focused provider or `translateForLanguage` boundary. Offline legal documents remain under their separately reviewed `legal/desktop/{en,de,ru}` manifest.
- The C++ settings service remains authoritative for the persisted language. Product and text-editor renderers keep localized product copy behind a startup gate until `bridge.getStatus` or `settings.getLanguage` resolves, so a secondary window never commits an English fallback frame before the saved locale is known. After a successful `settings.setLanguage`, the Rust shell emits the typed global `fluxora:settings:language-changed` event with `{ language, operationId }`; `window.fluxora.settings.onLanguageChanged` propagates it to every open renderer window. The initiating renderer applies the requested locale optimistically and rolls back only if native persistence fails.
...
- `window.fluxora.settings.onLanguageChanged` is the renderer-safe cross-window subscription for the successful native language mutation. The Rust event is emitted only after the C++ save response succeeds, carries the mutation `operationId`, and never replaces the authoritative response or startup read.
