...
- Downloads workspace: `downloads.list`, `downloads.importFile`, `downloads.delete`, `downloads.cancel`, `downloads.resume`, `downloads.resolveDuplicateDecision`, `nxm.registerProtocol`, `nxm.captureLinks` and `nxm.importInboundDownloads` are wired through Tauri Rust shell/facade and the native bridge host to C++ `DownloadService`. C++ proves exact Nexus update lineage, persists and revalidates duplicate snapshots, protects archives and finalizes replace/keep/cancel; renderer owns search, selection, sequential accessible decision dialogs, row context menus, double-click install trigger, local scroll windowing, selected-download details and platform capability messages only.
