# Fluxora Electron + C++ bridge architecture

Дата решения: 2026-06-24

Статус: Phase 14 Bridge/API surface and cross-platform capability model implemented on top of the Phase 1 decision. This document is the bridge/source-of-truth companion to `docs/electron-migration/wpf-ui-inventory.md` and `docs/electron-migration/cross-platform-support.md`.

## Decision summary

Fluxora will use a separate typed native bridge host between Electron main process and the C++ core:
...
The renderer never loads native modules, never receives direct Node.js access, never performs filesystem mutations, and never owns domain decisions. Electron contains UI state, window/app lifecycle, safe IPC exposure, native dialogs, shell-open behavior, and typed orchestration only. C++ remains the only owner of project, mod, plugin, download, install, VFS, FluxPack, Nexus, profile, executable, filesystem and operation behavior.
...
- Electron main service: `NativeBridgeService` starts the host, performs handshake, owns request metadata and writes Electron main/bridge logs.
- Preload API: renderer sees only `window.fluxora.bridge.getStatus`, `getLanguage`, `setLanguage` and `shutdown`.
- Renderer startup creates an `operationId`, writes a separate Electron UI log entry and shows either `Native bridge ready` or a clear fallback error.
...
- `operations.cancel` is present in the protocol MVP and returns `unsupported` until cancellable core operations are wired.
...
- Electron main/preload expose typed `window.fluxora.templates.*` and `window.fluxora.projects.*` calls only; renderer still has no Node.js, filesystem or raw IPC access.
- Electron main owns native file/folder dialogs, shell-open and frameless window controls through allowlisted preload APIs.
...
- Electron main/preload expose typed `window.fluxora.mods.*` calls only; renderer still has no Node.js, filesystem or raw IPC access.
...
- Electron main/preload expose typed `window.fluxora.plugins.*` calls only; renderer still has no Node.js, filesystem or raw IPC access.
...
- Electron main/preload expose typed `window.fluxora.downloads.*`, `window.fluxora.archives.install` and `window.fluxora.nxm.*` calls only; renderer still has no Node.js, filesystem, shell or raw IPC access.
- Electron main owns `nxm://` app activation handling through startup argv, Windows/Linux `second-instance` and macOS `open-url`, then forwards links to the bridge inbound queue.
...
- Electron main/preload expose typed install-analysis and FOMOD methods only; renderer still has no Node.js, filesystem, shell, native module or raw IPC access.
...
- Electron main/preload expose typed `window.fluxora.profiles.*` and `window.fluxora.executables.*` calls only; renderer still has no Node.js, filesystem, shell, native module or raw IPC access.
...
- Native host routes `settings.getTheme`, `settings.setTheme`, `nexus.getAuthStatus`, `nexus.connect`, `nexus.disconnect`, `transfer.analyzeMo2`, `transfer.importMo2` and `operations.cancel` to existing C++ C ABI functions backed by `AppSettingsService`, `NexusModsAuthService` and `ModOrganizerImportService`.
- Native host emits `operations.progress` JSON-RPC events during MO2 import. Electron main subscribes through the bridge client and broadcasts them on the allowlisted `fluxora:operations:progress` channel.
- Electron main/preload expose typed `window.fluxora.settings.*`, `window.fluxora.nexus.*`, `window.fluxora.transfer.*` and `window.fluxora.operations.*` calls only; renderer still has no Node.js, filesystem, shell, native module or raw IPC access.
- Renderer owns settings section state, language/theme controls, theme mirroring into CSS, Nexus status display, MO2 source/destination form state, analysis display, transfer progress display and route/close guard while transfer is running.
- C++ core remains the owner of persisted app settings, Nexus OAuth status/connect/disconnect behavior, MO2 analysis/import rules, disk-space checks, project creation/replacement and filesystem mutation.
- Operation cancellation remains capability-reported as unsupported until cancellable C++ operations are wired. The UI keeps the cancel command visible but reports the bridge capability result honestly.
...
- Electron main/preload expose typed `window.fluxora.buildPaths.*`, `window.fluxora.fluxPack.*` and `.fluxpack` native open/save dialogs only; renderer still has no Node.js, filesystem, shell, native module or raw IPC access.
...
- Operation cancellation remains capability-reported as unsupported until cancellable C++ operations are wired. Build creation/deletion and FluxPack overlays show close/cancel rules honestly: close is disabled while running, and cancel is disabled unless the bridge capability reports support.
...
- Electron UI: `fluxora-electron-ui-YYYYMMDD.log`
- Electron main/bridge: `fluxora-electron-main-bridge-YYYYMMDD.log`
...
- Electron security and native module documentation from Context7:
  - secure BrowserWindow/preload defaults with `contextIsolation`
  - `contextBridge` as the safe renderer exposure point
  - wrapped IPC methods instead of exposing `ipcRenderer`
  - Electron native modules require Electron ABI rebuilds
...
- Electron main process imports a `.node` addon.
...
- Electron native modules must match the Electron Node/V8 ABI and be rebuilt or prebuilt per Electron version, OS and architecture.
- A native crash can take down the Electron app process.
- Packaging gets tied to Electron's runtime details instead of Fluxora's core ABI.
...
- Electron main starts `FluxoraBridgeHost`.
...
- Electron is not coupled to Node native addon ABI.
...
### Option C: Local IPC through named pipe / Unix domain socket

Shape:

- Electron main connects to a local named pipe on Windows and Unix domain socket on Linux/macOS.
...
- Electron main or a temporary helper mimics the current C# `CoreBridgeService` by calling exported C ABI functions directly.
...
- Does not solve Electron ABI/security boundaries by itself.
...
- Existing `CoreBridgeService` and `FluxoraCoreApi.hpp` are the method and DTO inventory, not the new Electron boundary.

## Chosen architecture

### Responsibility boundary

Electron renderer owns:
...
Electron preload owns:

- A small `window.fluxora` API exposed through `contextBridge`.
- Argument/callback wrapping so renderer never sees `ipcRenderer` or Node primitives.
- Runtime shape validation before forwarding renderer calls to main.

Electron main owns:

- BrowserWindow lifecycle, app startup/shutdown and single-instance behavior.
...
- DTO validation at the Electron/main boundary.
- Mapping Electron errors into renderer-safe errors.
...
- A preload script that exposes only typed, allowlisted methods through `contextBridge`.
- No raw `ipcRenderer`, `ipcRenderer.on`, `ipcRenderer.invoke`, `shell`, `fs`, `path` or `child_process` exposure to renderer.
- Async IPC only. No `sendSync`.
...
These rules follow the current Electron guidance reviewed for Phase 1. They are acceptance criteria for Phase 2 bootstrap, but the architecture is already shaped around them here.
...
- Electron main creates or propagates operation IDs.
...
- UI must show honest capability state. A cancel button is disabled or hidden when the method cannot cancel that operation.
- Bridge v1 cancellation is mandatory in the contract even where the initial C++ implementation only returns `unsupported`.
...
- Electron main refuses to continue if there is no compatible protocol.
  ...
  "supportMatrix": [
    {
      ...
      "protocolNotes": "NXM uses Electron activation plus Windows registry verification.",
...
Capability truth comes from Electron main only for UI-shell features such as dialogs and shell open. Domain capabilities come from native bridge/core.

Phase 14 extends the exposed capability DTO with `supportMatrix`, a renderer-safe Windows/Linux/macOS readiness table. Electron main merges this table and main-owned feature states into the native bridge response before preload exposes `NativeBridgeStatus`. The renderer may display this matrix and disabled/limited states, but it must not invent domain support from the current OS string.
...
- Electron main locates the host through packaged app resources, not current working directory assumptions.
...
| Shell open/show item | `shell.openPath` / show in folder | `shell.openPath` / xdg behavior | `shell.openPath` / Finder behavior | Electron main |
| Native file/folder dialogs | Available | Available | Available | Electron main |
...
- Main/preload may open dialogs and return selected paths, but it does not normalize project semantics.
...
- Main validates path or URL schemes and calls Electron shell APIs.
...
- Electron main owns app activation and single-instance forwarding.
...
Bridge logs must be separate from UI logs. Do not merge Electron renderer console noise into core or operations logs.
