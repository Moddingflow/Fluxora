# Fluxora WPF UI inventory for Electron migration
...
Эта карта фиксирует текущий WPF интерфейс как контракт паритета для `frontend-electron/`. Инвентаризация сделана по исходникам:
...
- Electron UI: renderer state, route, component, view model/store, table/tree/dialog state.
- Electron main/preload: window lifecycle, native dialogs, protocol/deep links, shell open, safe IPC exposure.
- C++ core: domain behavior, file operations, installer, downloads, profiles, FluxPack, MO2 import, VFS/launch, logs.

Electron renderer must not duplicate C++ behavior. Existing WPF service classes mostly act as UI orchestration over `CoreBridgeService`; Electron should keep the same boundary through typed IPC.

## WPF entrypoints and Electron targets

| WPF entrypoint | Current role | Electron target | Owner notes |
| --- | --- | --- | --- |
...
| `StartupSplashWindow.xaml` + `StartupSplashViewModel` | Startup progress with minimum display duration and startup failure surfacing. | `routes/startup` or shell-level `StartupOverlay`. | Renderer visual. Main/preload exposes startup progress events. |
...
| WPF command | Current behavior | Electron target | Owner |
| --- | --- | --- | --- |
| `OpenCreateProjectCommand` | Opens embedded create wizard, resets inputs. | Navigate `/builds/new`. | Electron UI. |
| `BrowseGamePathCommand` | WPF executable picker for game exe. | `preload.dialog.pickExecutable`. | Electron main/preload. |
| `BrowseInstallRootCommand` | WPF folder picker. | `preload.dialog.pickFolder`. | Electron main/preload. |
| `PreviousCreateStepCommand`, `NextCreateStepCommand`, `CancelCreateProjectCommand` | Wizard navigation/validation. | Create-project wizard store. | Electron UI. |
...
| `OpenProjectCommand` | File picker for build config, opens project. | `dialog.pickBuildConfig` + `bridge.projects.openConfig`. | Main/preload + C++ core. |
| `OpenProjectBuildCommand` | Opens selected project from catalog. | `bridge.projects.open`. | Electron UI selection; C++ core read/validate. |
...
| `BackToProjectsCommand` | Leaves workspace and returns to catalog. | Navigate `/builds`. | Electron UI. |
...
| `OpenProjectDirectoryCommand`, `OpenGameDirectoryCommand`, `OpenHomeProjectDirectoryCommand`, `OpenHomeGameDirectoryCommand`, `OpenModsDirectoryCommand`, `OpenProfilesDirectoryCommand`, `OpenDownloadsDirectoryCommand` | Shell-open folders; creates downloads folder when needed. | `preload.shell.openPath`. | Electron main/preload. |
| `OpenProfileManagerCommand` | Opens profile manager dialog. | `ProfileManagerDialog` or route. | Electron UI. |
| `ToggleProfileMenuCommand` | Opens profile dropdown. | Renderer menu/popover. | Electron UI. |
...
| `OpenModInExplorerCommand` | Shell-open installed mod folder. | `preload.shell.openPath`. | Electron main/preload. |
| `ToggleModSeparatorCommand` | Collapse/expand separator locally. | Renderer state. | Electron UI. |
...
| `TogglePluginSeparatorCommand` | Collapse/expand separator locally. | Renderer state. | Electron UI. |
...
| `OpenDownloadInExplorerCommand` | Shell-open file or downloads folder. | `preload.shell.showItemInFolder/openPath`. | Electron main/preload. |
| `RegisterNxmProtocolCommand` | Registers current user NXM handler. | `main.protocol.registerNxm` plus core capability if retained. | Electron main/preload, C++ core if existing API remains source. |
...
| `CloseTransferPanelCommand`, `CloseBuildCreationProcessCommand`, `CloseBuildDeletionProcessCommand`, `CloseFluxPackPackageProcessCommand`, `CloseFluxPackInstallProcessCommand`, `CloseExecutableLaunchProcessCommand` | Close overlays when allowed. | Overlay store commands. | Electron UI. |
...
| WPF command/control | Current behavior | Electron target | Owner |
| --- | --- | --- | --- |
| `SelectConnectionsCommand`, `SelectLanguagesCommand`, `SelectCustomizationCommand`, `SelectTransferCommand` | Settings section navigation disabled during transfer process. | `/settings/connections`, `/settings/language`, `/settings/customization`, `/settings/transfer`. | Electron UI. |
...
| `OpenModOrganizerTransferCommand` | Opens MO2 transfer wizard. | `/transfer/mo2`. | Electron UI. |
| `BrowseSourceCommand`, `BrowseDestinationCommand` | Folder pickers for MO2 source/destination. | `preload.dialog.pickFolder`. | Electron main/preload. |
| `PreviousTransferStepCommand`, `NextTransferStepCommand`, `SelectTransferStepCommand`, `CancelTransferFlowCommand` | Transfer wizard navigation and cancellation before running. | Transfer wizard store. | Electron UI. |
...
| Settings close rule | Blocks close while transfer is running. | Modal route guard. | Electron UI/main window behavior. |
| Transfer to main button | Closes settings and opens transfer in main shell. | Navigate `/transfer/mo2`. | Electron UI. |
...
- `BrowseGameExecutableCommand`: Electron main/preload file picker.
- `BrowseModsDirectoryCommand`, `BrowseProfilesDirectoryCommand`, `BrowseDownloadsDirectoryCommand`, `BrowseOverwriteDirectoryCommand`: Electron main/preload folder pickers.
...
Electron target: `BuildPathsDialog` with explicit save/cancel, path validation, native browse controls and error state.
...
Electron target: `ProfileManagerDialog` or `/builds/:id/profiles`.

Owners:

- Electron UI: profile list, action name input, selected profile, busy state, validation messages.
- Electron main/preload: open profiles directory.
...
- Electron UI: list/editor/dropdown and launch overlay.
- Electron main/preload: pick executable/folder, shell/process tracking if moved out of renderer.
- C++ core: get/save executables, resolve icon path or expose icon data, launch executable with profile.

Cross-platform note: current validation is Windows `.exe`-centric. Electron migration must convert this to platform capability/validation rather than hiding unsupported platforms in UI logic.
...
The following `CoreBridgeService` methods are part of WPF UI parity and should be represented in the Electron bridge contract or intentionally replaced by equivalent methods:
