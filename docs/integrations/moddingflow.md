...
- A resumed ModdingFlow transfer re-resolves the stable artifact and performs a fresh HEAD before sending a Range request.
- Range append requires a compatible same-provider representation validator. An ETag is never reused across failover providers.
...
- The final archive is not promoted from its partial path until exact size and SHA-256 verification pass. Existing archive/path-safety and malware eligibility checks remain mandatory.
