...
| Update discovery | Application startup, a 15-minute primary-window schedule, focus return after at least five minutes, a manual Settings action, and the authorised post-install Setup flow request the fixed public signed manifest and signature. Ordinary IP, time and protocol metadata are visible to GitHub. | At most the two newest verified local manifest-cache records, plus local operation logs. | GitHub and content-delivery providers. | Security/compatibility legitimate interest and the Setup Install contract step, subject to counsel review. Automatic and manual discovery do not download a package; the in-app package action remains optional. | Privacy §§4, 8–10 |
...
| ModdingFlow account and downloads | PKCE sign-in in the system browser; account UUID/public profile/scopes/token expiry; rotating credential; catalogue, dependency, grant, job, artifact and short-lived transport identifiers. | Access/identity tokens in memory. Refresh credential in Windows Credential Manager until disconnect, invalidation, replacement or reset. Transfer sidecars retain stable identifiers/hashes, not the signed transport URL as durable identity. | ModdingFlow and its infrastructure/download providers. | User-requested account, catalogue or download function. Disconnect and credential removal controls are required. | Privacy §§5–6, 8–11 |
