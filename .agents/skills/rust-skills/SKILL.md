...
## Fluxora Tauri Shell Constraint

In `C:\Fluxora`, apply this skill only inside the existing Tauri Rust shell/facade ownership boundary. Rust should stay a thin shell for app lifecycle, safe commands, native dialogs, external links, window controls, bridge-host lifecycle, capabilities, and validated handoff to the C++ core. Do not move Fluxora business logic, installer rules, mod-management behavior, file-system domain decisions, archive semantics, profile/project rules, deploy/rollback behavior, or VFS logic from `backend/` into Rust just to make the Rust code more idiomatic.
